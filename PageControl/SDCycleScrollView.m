...
- (void)automaticScroll:(int)currentPage
{
...
- (void)layoutSubviews
{
    ...
    if (_isTradeBanner || _isTeamBanner || _isFullScreenWidth) {
        self.pageControl.frame = CGRectMake(x, y, size.width, size.height);
    }
    else{
        self.pageControl.frame = CGRectMake(x, y+33, size.width, size.height);
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.imageTag = (int)itemIndex +100;
    if (_isTradeBanner) {
        cell.isTradeBanner = YES;
    }
    if (_isTeamBanner) {
        cell.isTeamBanner = YES;
    }
    if (_isFullScreenWidth) {
        cell.isFullScreenWidth = YES;
    }
    NSString *imagePath = self.imagePathsGroup[itemIndex];
    
    if ([imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:[UIImage imageNamed:@"zwtu"] options:SDWebImageDelayPlaceholder];
        }
        else {
    ...
    cell.imageView.tag = (int)itemIndex +100;
    return cell;
}
