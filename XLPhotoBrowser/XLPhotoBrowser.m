...
@property (nonatomic, strong) UIButton *useCoverBtn;
/**
 *   保存图片的过程指示菊花
 */
...
- (void)setUpUseCoverButton {
    _useCoverBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _useCoverBtn.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 120 - 15, 15, 120, 48);
    _useCoverBtn.backgroundColor = RGB_COLOR(0, 0, 0, 0.4);
    [_useCoverBtn setTitleColor:RGB_COLOR(221, 51, 51, 1.0) forState:UIControlStateNormal];
    [_useCoverBtn setTitle:@"设为封面" forState:UIControlStateNormal];
    [_useCoverBtn addTarget:self action:@selector(usageCoverImage) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:_useCoverBtn];
}

- (void)usageCoverImage {
    HCTopTipView *tipView = [[HCTopTipView alloc] init];
    [tipView showTopSuccessWithDetail:@"此图片已设置为封面" addView:self];
    if (self.delegate && [self.delegate respondsToSelector:@selector(UsageCoverWithCurrentImageIndex:)]) {
        [self.delegate UsageCoverWithCurrentImageIndex:self.currentImageIndex];
    }
}
